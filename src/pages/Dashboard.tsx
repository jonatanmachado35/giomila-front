  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        {(loading || error) && (
          <div className="rounded-lg border p-4 flex items-center justify-between bg-card/80">
            <p className="text-sm text-muted-foreground">
              {loading ? "Carregando dados em tempo real do Supabase..." : `Erro ao carregar dados: ${error}`}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <MonthlyRanking
            data={data.monthlyRanking.slice(0, 4)}
            title="Ranking dos Últimos Meses"
          />
          <FatigueChart
            data={data.fatigueByHour}
